...
public class Ex1322_Nemonemo extends Ex1322_CloseableFrame // JFrame으로부터 상속받은 CloseableFrame 상속
  implements ActionListener
{
  ...
  {
    Ex1322_Nemonemo nemo= new Ex1322_Nemonemo(); // 네모네모로직 게임 생성
  ...
  public void display() // 퍼즐이 풀렸는지 여부를 검사
  {
    ...
    for(int j=0; (j<10)&&endFlag; j++)
      ...
      {
        if((data.charAt(j*10+i)=='1')&&(temp[j*10+i]!=1)) endFlag=false; // 채웡할 칸을 모두 채웠는지 검사
        else if((data.charAt(j*10+i)!='1')&&(temp[j*10+i]==1)) endFlag=false; // 채우지 않아야 할 칸을 채웠는지 검사
      }
